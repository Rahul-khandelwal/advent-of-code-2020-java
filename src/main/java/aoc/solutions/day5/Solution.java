...
public class Solution {
    ...
    public static void main(String[] args) {
        part2_assignedSeatId();
    }

    private static void part1_highestSeatId() {
        System.out.println(seatIds().max().getAsInt());
    }

    private static void part2_assignedSeatId() {
        var existingSeatIds = seatIds().boxed().collect(Collectors.toSet());

        var assignedSeat = IntStream.range(seatIds().min().getAsInt(), seatIds().max().getAsInt())
                .boxed()
                .filter(not(existingSeatIds::contains))
                .findAny()
                .orElseThrow();

        System.out.println(assignedSeat);
    }

    private static IntStream seatIds() {
        return Arrays.stream(input())
                .mapToInt(Solution::seatId);
    }
    ...
    private static int seatId(String boardingPass) {
        int lowestRow = 0;
            ...
            int i;
            for (i = 0 ; i < 7 ; i++) {
                int mid = lowestRow + (highestRow - lowestRow) / 2;
            ...
            while (i < boardingPass.length()) {
                int mid = lowestSeat + (highestSeat - lowestSeat) / 2;
