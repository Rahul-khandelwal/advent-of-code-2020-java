...
public class Solution {
    ...
    public static void main(String[] args) {
        part2_validPassports();
    }

    private static void part1_validPassports() {

        int valid = 0;

        for (Map<String, String> passportData : input()) {
            if (passportData.size() <= 6) {
                continue;
            }
    ...
    private static void part2_validPassports() {
        ...
        for (Map<String, String> passportData : input()) {
            ...
            for (Map.Entry<String, Function<String, Boolean>> rule : rules.entrySet()) {
                if (!rule.getValue().apply(passportData.get(rule.getKey()))) {
                    isValid = false;
                    break;
                }
            }
    ...
    private static List<Map<String, String>> input() {
        ...
        for (String line : inputStr.split("\\n")) {
            if (line.isBlank()) {
                inputs.add(passwordData(currData));
                currData.clear();
            } else {
                currData.add(line);
            }
    ...
    private static Map<String, String> passwordData(List<String> currData) {
        if (currData.isEmpty()) {
    ...
    private static Map<String, Function<String, Boolean>> validationRule() {
        return Map.of(
                "byr", (val) -> {
                    if (val == null || val.isBlank() || val.length() != 4) {
                        return false;
                    }
                ...
                "hgt", (val) -> {
                    ...
                    try {
                        var num = Integer.parseInt(val.substring(0, val.length() - 2));
                        if (val.endsWith("cm")) {
                ...
                "hcl", (val) -> {
                    if (val == null || val.isBlank() || val.length() != 7) {
                        return false;
                    }
                ...
                "ecl", (val) -> {
                    ...
                    return val.matches("^(amb|blu|brn|gry|grn|hzl|oth)$");
                },
                "pid", (val) -> {
                    if (val == null || val.isBlank() || val.length() != 9) {
                        return false;
                    }
